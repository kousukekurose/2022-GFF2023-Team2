...
public class LeaderEnemy : MonoBehaviour
{
    ...
    #region プレイヤーから攻撃を受けたときの処理
    //エネミーが攻撃を受ける
    public void LeaderEnemyDamage(int damage)
    {
        Hp -= damage;
        if (Hp <= 0)
        {
    ...
    {
        StartCoroutine(LeaderEnemyDie());
    }
    IEnumerator LeaderEnemyDie()
    {
        Hp = 0;
    ...
    //攻撃範囲に入っているかどうか確認
    public void Area(GameObject player)
    {
        distance = Vector3.Distance(player.GetComponent<Player>().transform.position, this.transform.position);
        //PlayerがtrackingRangeより近づいたら追跡開始
